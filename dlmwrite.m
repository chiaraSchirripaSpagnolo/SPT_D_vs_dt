...
% open the file
if append
    fid = openw(filename ,'Ab');
else
    fid = openw(filename ,'Wb');
end
...
if ischar(in)
    ...
    elseif strcmpi(in,'unix')
        ...
        error('MATLAB:dlmwrite:newline',...
          'Newline must be either ''pc'' or ''unix''.');
    end
else
    error('MATLAB:dlmwrite:newline',...
    'Newline must be either ''pc'' or ''unix''.');
...
if isnumeric(in)
    ...
    error('MATLAB:dlmwrite:rowOffset',...
        '%s is not a valid attribute or row offset.',in);
end
%------------------------------------------------------------------------------
...
if isnumeric(in)
    ...
    error('MATLAB:dlmwrite:columnOffset',...
        '%s is not a valid attribute or column offset.',in);
end
%-----------------------------------------------------------------------
